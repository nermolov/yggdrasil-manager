...
import (
	...
	ipv6rwcInternal "github.com/yggdrasil-network/yggdrasil-go/src/ipv6rwc"
)
...
func (k *keyStore) readPC(p []byte) (int, error) {
	...
	for {
		...
		if len(bs) > mtu {
			...
			if packet, err := ipv6rwcInternal.CreateICMPv6(buf[8:24], buf[24:40], ipv6.ICMPTypePacketTooBig, 0, ptb); err == nil {
